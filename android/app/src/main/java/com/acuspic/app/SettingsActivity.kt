...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun showUpdateDialog(result: UpdateCheckResult.HasUpdate) {
        ...
        val dialog = UpdateInfoDialog(this)
        dialog.setVersionInfo(versionInfo, result.isForceUpdate)
        
        dialog.setOnSkipListener {
            updateManager.skipVersion(versionInfo.versionCode)
        }
        
        dialog.setOnLaterListener {
            // 稍后提醒，直接关闭对话框
        }
        
        dialog.setOnDownloadListener {
    ...
    private fun showDownloadDialog(versionInfo: com.acuspic.app.update.VersionInfo) {
        ...
        dialog.setTotalSize(versionInfo.fileSize)
        dialog.show()
        ...
        lifecycleScope.launch {
            updateManager.downloadStatus.collect { status ->
                when (status) {
                    is com.acuspic.app.update.DownloadStatus.Connecting -> {
                        dialog.setConnecting()
                    }
        ...
        dialog.setOnBackgroundListener {
            Toast.makeText(this@SettingsActivity, "下载将在后台继续", Toast.LENGTH_SHORT).show()
        }
    }
